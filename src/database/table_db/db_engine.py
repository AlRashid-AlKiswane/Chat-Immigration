...
def get_sqlite_engine() -> Optional[sqlite3.Connection]:
    ...
    try:
        ...
        try:
            ...
            logger.debug("Verified database directory: %s", db_dir)
        except PermissionError as pe:
            logger.error("Permission denied creating directory: %s", pe)
            raise sqlite3.OperationalError(f"Directory creation failed: {pe}") from pe

        # Verify directory is writable
        if not os.access(db_dir, os.W_OK):
            logger.error("Database directory not writable: %s", db_dir)
        ...
        try:
            ...
            logger.info("Successfully connected to database: %s", db_path)
            return conn
        except sqlite3.Error as se:
            logger.error("Database connection failed: %s", se)
            raise
        ...
        logger.error("Database operation failed: %s", se)
        raise
